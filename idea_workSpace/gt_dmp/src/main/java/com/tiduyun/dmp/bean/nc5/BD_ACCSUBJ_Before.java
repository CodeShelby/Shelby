package com.tiduyun.dmp.bean.nc5;

public class BD_ACCSUBJ_Before {
    private String ACCREMOVE;
    private String BALANFLAG;
    private String BALANORIENT;
    private String BEGINPERIOD;
    private String BEGINYEAR;
    private String BOTHORIENT;
    private String CASHBANKFLAG;
    private String CREATECORP;
    private String 	CREATEPERIOD;
    private String 	CREATEYEAR;
    private String 	CTLSYSTEM;
    private String 	CURRENCY;
    private String 	DISPNAME;
    private String 	DR;
    private String 	ENDFLAG;
    private String 	ENDPERIOD;
    private String 	ENDYEAR;
    private String 	ENGSUBJNAME;
    private String 	FREE1;
    private String 	FREE10;
    private String 	FREE11;
    private String 	FREE12;
    private String 	FREE13;
    private String 	FREE14;
    private String 	FREE15;
    private String 	FREE16;
    private String 	FREE17;
    private String 	FREE18;
    private String  FREE19;
    private String	FREE2;
    private String	FREE20;
    private String	FREE3;
    private String	FREE4;
    private String	FREE5;
    private String	FREE6;
    private String	FREE7;
    private String	FREE8;
    private String	FREE9;
    private String	INCURFLAG;
    private String	INNERINFONULL;
    private String	INNERSUBJ;
    private String	OUTFLAG;
    private String	PK_ACCSUBJ;
    private String	PK_CORP;
    private String	PK_CREATE_GLORGBOOK;
    private String	PK_GLORGBOOK;
    private String	PK_GRPACCSUBJ;
    private String	PK_SUBJSCHEME;
    private String	PK_SUBJTYPE;
    private String  PROPERTY1;
    private String	PROPERTY2;
    private String	PROPERTY3;
    private String	PROPERTY4;
    private String	PROPERTY5;
    private String	REMCODE;
    private String	SEALFLAG;
    private String	STOPED;
    private String	SUBJCODE;
    private String	SUBJLEV;
    private String	SUBJNAME;
    private String	SUMPRString_LEVEL;
    private String	TS;
    private String	UNIT;

    public String getACCREMOVE() {
        return ACCREMOVE;
    }

    public void setACCREMOVE(String ACCREMOVE) {
        this.ACCREMOVE = ACCREMOVE;
    }

    public String getBALANFLAG() {
        return BALANFLAG;
    }

    public void setBALANFLAG(String BALANFLAG) {
        this.BALANFLAG = BALANFLAG;
    }

    public String getBALANORIENT() {
        return BALANORIENT;
    }

    public void setBALANORIENT(String BALANORIENT) {
        this.BALANORIENT = BALANORIENT;
    }

    public String getBEGINPERIOD() {
        return BEGINPERIOD;
    }

    public void setBEGINPERIOD(String BEGINPERIOD) {
        this.BEGINPERIOD = BEGINPERIOD;
    }

    public String getBEGINYEAR() {
        return BEGINYEAR;
    }

    public void setBEGINYEAR(String BEGINYEAR) {
        this.BEGINYEAR = BEGINYEAR;
    }

    public String getBOTHORIENT() {
        return BOTHORIENT;
    }

    public void setBOTHORIENT(String BOTHORIENT) {
        this.BOTHORIENT = BOTHORIENT;
    }

    public String getCASHBANKFLAG() {
        return CASHBANKFLAG;
    }

    public void setCASHBANKFLAG(String CASHBANKFLAG) {
        this.CASHBANKFLAG = CASHBANKFLAG;
    }

    public String getCREATECORP() {
        return CREATECORP;
    }

    public void setCREATECORP(String CREATECORP) {
        this.CREATECORP = CREATECORP;
    }

    public String getCREATEPERIOD() {
        return CREATEPERIOD;
    }

    public void setCREATEPERIOD(String CREATEPERIOD) {
        this.CREATEPERIOD = CREATEPERIOD;
    }

    public String getCREATEYEAR() {
        return CREATEYEAR;
    }

    public void setCREATEYEAR(String CREATEYEAR) {
        this.CREATEYEAR = CREATEYEAR;
    }

    public String getCTLSYSTEM() {
        return CTLSYSTEM;
    }

    public void setCTLSYSTEM(String CTLSYSTEM) {
        this.CTLSYSTEM = CTLSYSTEM;
    }

    public String getCURRENCY() {
        return CURRENCY;
    }

    public void setCURRENCY(String CURRENCY) {
        this.CURRENCY = CURRENCY;
    }

    public String getDISPNAME() {
        return DISPNAME;
    }

    public void setDISPNAME(String DISPNAME) {
        this.DISPNAME = DISPNAME;
    }

    public String getDR() {
        return DR;
    }

    public void setDR(String DR) {
        this.DR = DR;
    }

    public String getENDFLAG() {
        return ENDFLAG;
    }

    public void setENDFLAG(String ENDFLAG) {
        this.ENDFLAG = ENDFLAG;
    }

    public String getENDPERIOD() {
        return ENDPERIOD;
    }

    public void setENDPERIOD(String ENDPERIOD) {
        this.ENDPERIOD = ENDPERIOD;
    }

    public String getENDYEAR() {
        return ENDYEAR;
    }

    public void setENDYEAR(String ENDYEAR) {
        this.ENDYEAR = ENDYEAR;
    }

    public String getENGSUBJNAME() {
        return ENGSUBJNAME;
    }

    public void setENGSUBJNAME(String ENGSUBJNAME) {
        this.ENGSUBJNAME = ENGSUBJNAME;
    }

    public String getFREE1() {
        return FREE1;
    }

    public void setFREE1(String FREE1) {
        this.FREE1 = FREE1;
    }

    public String getFREE10() {
        return FREE10;
    }

    public void setFREE10(String FREE10) {
        this.FREE10 = FREE10;
    }

    public String getFREE11() {
        return FREE11;
    }

    public void setFREE11(String FREE11) {
        this.FREE11 = FREE11;
    }

    public String getFREE12() {
        return FREE12;
    }

    public void setFREE12(String FREE12) {
        this.FREE12 = FREE12;
    }

    public String getFREE13() {
        return FREE13;
    }

    public void setFREE13(String FREE13) {
        this.FREE13 = FREE13;
    }

    public String getFREE14() {
        return FREE14;
    }

    public void setFREE14(String FREE14) {
        this.FREE14 = FREE14;
    }

    public String getFREE15() {
        return FREE15;
    }

    public void setFREE15(String FREE15) {
        this.FREE15 = FREE15;
    }

    public String getFREE16() {
        return FREE16;
    }

    public void setFREE16(String FREE16) {
        this.FREE16 = FREE16;
    }

    public String getFREE17() {
        return FREE17;
    }

    public void setFREE17(String FREE17) {
        this.FREE17 = FREE17;
    }

    public String getFREE18() {
        return FREE18;
    }

    public void setFREE18(String FREE18) {
        this.FREE18 = FREE18;
    }

    public String getFREE19() {
        return FREE19;
    }

    public void setFREE19(String FREE19) {
        this.FREE19 = FREE19;
    }

    public String getFREE2() {
        return FREE2;
    }

    public void setFREE2(String FREE2) {
        this.FREE2 = FREE2;
    }

    public String getFREE20() {
        return FREE20;
    }

    public void setFREE20(String FREE20) {
        this.FREE20 = FREE20;
    }

    public String getFREE3() {
        return FREE3;
    }

    public void setFREE3(String FREE3) {
        this.FREE3 = FREE3;
    }

    public String getFREE4() {
        return FREE4;
    }

    public void setFREE4(String FREE4) {
        this.FREE4 = FREE4;
    }

    public String getFREE5() {
        return FREE5;
    }

    public void setFREE5(String FREE5) {
        this.FREE5 = FREE5;
    }

    public String getFREE6() {
        return FREE6;
    }

    public void setFREE6(String FREE6) {
        this.FREE6 = FREE6;
    }

    public String getFREE7() {
        return FREE7;
    }

    public void setFREE7(String FREE7) {
        this.FREE7 = FREE7;
    }

    public String getFREE8() {
        return FREE8;
    }

    public void setFREE8(String FREE8) {
        this.FREE8 = FREE8;
    }

    public String getFREE9() {
        return FREE9;
    }

    public void setFREE9(String FREE9) {
        this.FREE9 = FREE9;
    }

    public String getINCURFLAG() {
        return INCURFLAG;
    }

    public void setINCURFLAG(String INCURFLAG) {
        this.INCURFLAG = INCURFLAG;
    }

    public String getINNERINFONULL() {
        return INNERINFONULL;
    }

    public void setINNERINFONULL(String INNERINFONULL) {
        this.INNERINFONULL = INNERINFONULL;
    }

    public String getINNERSUBJ() {
        return INNERSUBJ;
    }

    public void setINNERSUBJ(String INNERSUBJ) {
        this.INNERSUBJ = INNERSUBJ;
    }

    public String getOUTFLAG() {
        return OUTFLAG;
    }

    public void setOUTFLAG(String OUTFLAG) {
        this.OUTFLAG = OUTFLAG;
    }

    public String getPK_ACCSUBJ() {
        return PK_ACCSUBJ;
    }

    public void setPK_ACCSUBJ(String PK_ACCSUBJ) {
        this.PK_ACCSUBJ = PK_ACCSUBJ;
    }

    public String getPK_CORP() {
        return PK_CORP;
    }

    public void setPK_CORP(String PK_CORP) {
        this.PK_CORP = PK_CORP;
    }

    public String getPK_CREATE_GLORGBOOK() {
        return PK_CREATE_GLORGBOOK;
    }

    public void setPK_CREATE_GLORGBOOK(String PK_CREATE_GLORGBOOK) {
        this.PK_CREATE_GLORGBOOK = PK_CREATE_GLORGBOOK;
    }

    public String getPK_GLORGBOOK() {
        return PK_GLORGBOOK;
    }

    public void setPK_GLORGBOOK(String PK_GLORGBOOK) {
        this.PK_GLORGBOOK = PK_GLORGBOOK;
    }

    public String getPK_GRPACCSUBJ() {
        return PK_GRPACCSUBJ;
    }

    public void setPK_GRPACCSUBJ(String PK_GRPACCSUBJ) {
        this.PK_GRPACCSUBJ = PK_GRPACCSUBJ;
    }

    public String getPK_SUBJSCHEME() {
        return PK_SUBJSCHEME;
    }

    public void setPK_SUBJSCHEME(String PK_SUBJSCHEME) {
        this.PK_SUBJSCHEME = PK_SUBJSCHEME;
    }

    public String getPK_SUBJTYPE() {
        return PK_SUBJTYPE;
    }

    public void setPK_SUBJTYPE(String PK_SUBJTYPE) {
        this.PK_SUBJTYPE = PK_SUBJTYPE;
    }

    public String getPROPERTY1() {
        return PROPERTY1;
    }

    public void setPROPERTY1(String PROPERTY1) {
        this.PROPERTY1 = PROPERTY1;
    }

    public String getPROPERTY2() {
        return PROPERTY2;
    }

    public void setPROPERTY2(String PROPERTY2) {
        this.PROPERTY2 = PROPERTY2;
    }

    public String getPROPERTY3() {
        return PROPERTY3;
    }

    public void setPROPERTY3(String PROPERTY3) {
        this.PROPERTY3 = PROPERTY3;
    }

    public String getPROPERTY4() {
        return PROPERTY4;
    }

    public void setPROPERTY4(String PROPERTY4) {
        this.PROPERTY4 = PROPERTY4;
    }

    public String getPROPERTY5() {
        return PROPERTY5;
    }

    public void setPROPERTY5(String PROPERTY5) {
        this.PROPERTY5 = PROPERTY5;
    }

    public String getREMCODE() {
        return REMCODE;
    }

    public void setREMCODE(String REMCODE) {
        this.REMCODE = REMCODE;
    }

    public String getSEALFLAG() {
        return SEALFLAG;
    }

    public void setSEALFLAG(String SEALFLAG) {
        this.SEALFLAG = SEALFLAG;
    }

    public String getSTOPED() {
        return STOPED;
    }

    public void setSTOPED(String STOPED) {
        this.STOPED = STOPED;
    }

    public String getSUBJCODE() {
        return SUBJCODE;
    }

    public void setSUBJCODE(String SUBJCODE) {
        this.SUBJCODE = SUBJCODE;
    }

    public String getSUBJLEV() {
        return SUBJLEV;
    }

    public void setSUBJLEV(String SUBJLEV) {
        this.SUBJLEV = SUBJLEV;
    }

    public String getSUBJNAME() {
        return SUBJNAME;
    }

    public void setSUBJNAME(String SUBJNAME) {
        this.SUBJNAME = SUBJNAME;
    }

    public String getSUMPRString_LEVEL() {
        return SUMPRString_LEVEL;
    }

    public void setSUMPRString_LEVEL(String SUMPRString_LEVEL) {
        this.SUMPRString_LEVEL = SUMPRString_LEVEL;
    }

    public String getTS() {
        return TS;
    }

    public void setTS(String TS) {
        this.TS = TS;
    }

    public String getUNIT() {
        return UNIT;
    }

    public void setUNIT(String UNIT) {
        this.UNIT = UNIT;
    }

    @Override
    public String toString() {
        return "BD_ACCSUBJ{" +
                "ACCREMOVE='" + ACCREMOVE + '\'' +
                ", BALANFLAG='" + BALANFLAG + '\'' +
                ", BALANORIENT=" + BALANORIENT +
                ", BEGINPERIOD='" + BEGINPERIOD + '\'' +
                ", BEGINYEAR='" + BEGINYEAR + '\'' +
                ", BOTHORIENT='" + BOTHORIENT + '\'' +
                ", CASHBANKFLAG=" + CASHBANKFLAG +
                ", CREATECORP='" + CREATECORP + '\'' +
                ", CREATEPERIOD='" + CREATEPERIOD + '\'' +
                ", CREATEYEAR='" + CREATEYEAR + '\'' +
                ", CTLSYSTEM='" + CTLSYSTEM + '\'' +
                ", CURRENCY='" + CURRENCY + '\'' +
                ", DISPNAME='" + DISPNAME + '\'' +
                ", DR=" + DR +
                ", ENDFLAG='" + ENDFLAG + '\'' +
                ", ENDPERIOD='" + ENDPERIOD + '\'' +
                ", ENDYEAR='" + ENDYEAR + '\'' +
                ", ENGSUBJNAME='" + ENGSUBJNAME + '\'' +
                ", FREE1='" + FREE1 + '\'' +
                ", FREE10='" + FREE10 + '\'' +
                ", FREE11='" + FREE11 + '\'' +
                ", FREE12='" + FREE12 + '\'' +
                ", FREE13='" + FREE13 + '\'' +
                ", FREE14='" + FREE14 + '\'' +
                ", FREE15='" + FREE15 + '\'' +
                ", FREE16='" + FREE16 + '\'' +
                ", FREE17='" + FREE17 + '\'' +
                ", FREE18='" + FREE18 + '\'' +
                ", FREE19='" + FREE19 + '\'' +
                ", FREE2='" + FREE2 + '\'' +
                ", FREE20='" + FREE20 + '\'' +
                ", FREE3='" + FREE3 + '\'' +
                ", FREE4='" + FREE4 + '\'' +
                ", FREE5='" + FREE5 + '\'' +
                ", FREE6='" + FREE6 + '\'' +
                ", FREE7='" + FREE7 + '\'' +
                ", FREE8='" + FREE8 + '\'' +
                ", FREE9='" + FREE9 + '\'' +
                ", INCURFLAG='" + INCURFLAG + '\'' +
                ", INNERINFONULL='" + INNERINFONULL + '\'' +
                ", INNERSUBJ='" + INNERSUBJ + '\'' +
                ", OUTFLAG='" + OUTFLAG + '\'' +
                ", PK_ACCSUBJ='" + PK_ACCSUBJ + '\'' +
                ", PK_CORP='" + PK_CORP + '\'' +
                ", PK_CREATE_GLORGBOOK='" + PK_CREATE_GLORGBOOK + '\'' +
                ", PK_GLORGBOOK='" + PK_GLORGBOOK + '\'' +
                ", PK_GRPACCSUBJ='" + PK_GRPACCSUBJ + '\'' +
                ", PK_SUBJSCHEME='" + PK_SUBJSCHEME + '\'' +
                ", PK_SUBJTYPE='" + PK_SUBJTYPE + '\'' +
                ", PROPERTY1='" + PROPERTY1 + '\'' +
                ", PROPERTY2='" + PROPERTY2 + '\'' +
                ", PROPERTY3='" + PROPERTY3 + '\'' +
                ", PROPERTY4='" + PROPERTY4 + '\'' +
                ", PROPERTY5='" + PROPERTY5 + '\'' +
                ", REMCODE='" + REMCODE + '\'' +
                ", SEALFLAG='" + SEALFLAG + '\'' +
                ", STOPED='" + STOPED + '\'' +
                ", SUBJCODE='" + SUBJCODE + '\'' +
                ", SUBJLEV=" + SUBJLEV +
                ", SUBJNAME='" + SUBJNAME + '\'' +
                ", SUMPRString_LEVEL=" + SUMPRString_LEVEL +
                ", TS='" + TS + '\'' +
                ", UNIT='" + UNIT + '\'' +
                '}';
    }
}
